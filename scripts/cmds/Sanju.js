...
const obfuscatedAuthors = ["Obito", "Sanju"];
...
module.exports = {
  config: {
    ...
    aliases: ["obito", "sanju"],
    version: "1.0",
    author: "Obito",  // মূল লেখক হিসেবে Obito সেট করা হয়েছে
    countDown: 5,
    role: 0,
    description: {
  ...
  onReply: async function ({ api, event, Reply }) {
    try {
      if (!obfuscatedAuthors.includes(this.config.author)) {
        return api.sendMessage("You are not authorized to change the author name.\n\nPlease fix the author name to use this command.", event.threadID, event.messageID);
      }
